// src/app/admin/Collaborators/components/AddCollaboratorModal.tsx
"use client";
...
import {
  getCountryCallingCode, getExampleNumber, parsePhoneNumberFromString, isSupportedCountry,
...
/* === Generar contraseña temporal (12 chars, minúscula, mayúscula y número) === */
function genTempPassword(length = 12): string {
  ...
  const rest = Array.from({ length: restLen }, () => cryptoPick(all));
  const pwdArr = [...req, ...rest];
...
export default function AddCollaboratorModal({ open, mode, initial, onClose, onSaved }: Props) {
  ...
  /* Limpia mensajes/flags cada vez que se abre */
  useEffect(() => {
    if (open) {
      setOkMsg(null);
      setServerError(null);
  ...
  /* Precarga si es editar / reset si es crear */
  useEffect(() => {
  ...
  const errors = useMemo(() => {
    ...
    if (config.onlyNumbers && !/^\d+$/.test(id)) e.identification = "Solo números";
    else if (!config.pattern.test(id)) e.identification = `Formato inválido`;
    else if (id.length < config.length[0] || id.length > config.length[1]) e.identification = `Debe tener entre ${config.length[0]} y ${config.length[1]}`;
  ...
  function getMaxNationalLengthFor(country?: string): number | null {
    ...
    const fallback: Record<string, number> = { CR: 8, US: 10, CA: 10, MX: 10, ES: 9, AR: 10, CL: 9, CO: 10, PE: 9, BR: 11, EC: 9, PA: 8, NI: 8, HN: 8, SV: 8, GT: 8, DO: 10, PR: 10 };
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "editar" && initial?.id != null) {
        await save({ id: initial.id, ...basePayload });      // PATCH (id solo en URL)
        setOkMsg("✅ Cambios guardados.");
        onSaved?.("updated");
      } else {
        // Generar contraseña temporal automáticamente
        const tempPassword = genTempPassword(12);
        await save({ ...basePayload, password: tempPassword } as any); // POST
        setOkMsg("✅ Colaborador creado. Se generó una contraseña temporal.");
        onSaved?.("created");
        resetForm();
      }
