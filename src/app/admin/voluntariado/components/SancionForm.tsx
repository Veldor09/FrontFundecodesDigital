...
export default function SancionForm({ initial, onSave, onCancel, voluntarioPreseleccionado }: Props) {
  ...
  // Si hay un voluntario preseleccionado, deshabilitamos el selector
  useEffect(() => {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      const dto = {
        ...
        fechaVencimiento: data.esPermanente ? null : (data.fechaVencimiento ? new Date(data.fechaVencimiento).toISOString() : null),
  ...
  const voluntarioSeleccionado = voluntarios.find(v => v.id === watch("voluntarioId"));
      ...
      <div>
        ...
        {voluntarioPreseleccionado ? (
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-md">
            <p className="font-medium text-blue-900">{voluntarioPreseleccionado.nombreCompleto}</p>
            <p className="text-sm text-blue-700">{voluntarioPreseleccionado.numeroDocumento} • {voluntarioPreseleccionado.email}</p>
          </div>
        ) : (
          <>
            <select
            ...
            {errors.voluntarioId && (
              <p className="text-red-500 text-xs mt-1">{String(errors.voluntarioId.message)}</p>
            )}
          </>
        )}
      </div>
          ...
          <div className="space-y-2">
            ...
            {!esPermanente && (
              <Input
                ...
                min={watch("fechaInicio")} // No puede vencer antes de empezar
              />
            )}
          </div>
        </div>
      ...
      {voluntarioSeleccionado && (
        <div className="p-4 bg-slate-50 border border-slate-200 rounded-md">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm text-slate-600">
            <div><strong>Nombre:</strong> {voluntarioSeleccionado.nombreCompleto}</div>
            <div><strong>Documento:</strong> {voluntarioSeleccionado.numeroDocumento}</div>
            <div><strong>Email:</strong> {voluntarioSeleccionado.email}</div>
            <div><strong>Estado:</strong> <span className={voluntarioSeleccionado.estado === "ACTIVO" ? "text-green-600" : "text-red-600"}>{voluntarioSeleccionado.estado}</span></div>
          </div>
        </div>
      )}
